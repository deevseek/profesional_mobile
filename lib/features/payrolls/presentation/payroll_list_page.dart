...
class _PayrollListPageState extends State<PayrollListPage> {
  final PayrollController _controller = PayrollController();
            ...
            children: [
              ...
              Expanded(
                child: RefreshIndicator(
                  onRefresh: () async {
                    await _controller.loadPayrolls(
                      employee: _employeeController.text,
                      status: _statusController.text,
  ...
  Widget _buildList(BuildContext context) {
    if (_controller.isLoading && _controller.payrolls.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }

    if (_controller.payrolls.isEmpty) {
      return ListView(
    ...
    return ListView.separated(
      ...
      itemCount: _controller.payrolls.length,
      separatorBuilder: (_, __) => const SizedBox(height: 12),
      itemBuilder: (context, index) {
        final payroll = _controller.payrolls[index];
        final employeeLabel = payroll.employeeName ?? 'Employee';
        return Card(
          elevation: 1,
          child: ListTile(
            leading: CircleAvatar(
              child: Text(
                employeeLabel.isNotEmpty ? employeeLabel[0].toUpperCase() : '#',
              ),
            ),
            title: Text(employeeLabel.isNotEmpty ? employeeLabel : 'Employee'),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (payroll.period != null && payroll.period!.isNotEmpty)
                  Text('Period: ${payroll.period}'),
                if (payroll.grossAmount != null)
                  Text('Gross: ${_formatAmount(payroll.grossAmount)}'),
                if (payroll.netAmount != null)
                  Text('Net: ${_formatAmount(payroll.netAmount)}'),
                if (payroll.reference != null && payroll.reference!.isNotEmpty)
                  Text('Reference: ${payroll.reference}'),
                if (payroll.notes != null && payroll.notes!.isNotEmpty) Text(payroll.notes!),
                if (payroll.createdAt != null)
                  Text('Created: ${_formatDate(payroll.createdAt)}'),
                if (payroll.status != null && payroll.status!.isNotEmpty) ...[
                  const SizedBox(height: 6),
                  _buildStatusBadge(context, payroll.status!),
                ],
              ],
            ),
          ),
