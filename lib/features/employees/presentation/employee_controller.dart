import 'package:dio/dio.dart';
import 'package:flutter/foundation.dart';

import '../../../core/errors/api_exception.dart';
import '../data/employee_repository_impl.dart';
import '../domain/employee_model.dart';
import '../domain/employee_repository.dart';

class EmployeeController extends ChangeNotifier {
  EmployeeController({EmployeeRepository? repository})
      : _repository = repository ?? EmployeeRepositoryImpl();

  final EmployeeRepository _repository;

  bool _loading = false;
  bool _submitting = false;
  String? _errorMessage;
  String? _successMessage;
  List<Employee> _employees = const [];
  EmployeePaginationMeta? _meta;
  EmployeePaginationLinks? _links;
  String _searchQuery = '';
  int _page = 1;
  Employee? _employee;
  Map<String, List<String>> _fieldErrors = const {};

  bool get isLoading => _loading;
  bool get isSubmitting => _submitting;
  String? get errorMessage => _errorMessage;
  String? get successMessage => _successMessage;
  List<Employee> get employees => _employees;
  EmployeePaginationMeta? get meta => _meta;
  EmployeePaginationLinks? get links => _links;
  String get searchQuery => _searchQuery;
  int get page => _page;
  Employee? get employee => _employee;
  Map<String, List<String>> get fieldErrors => _fieldErrors;

  Future<void> loadEmployees({String? search, int page = 1}) async {
    _setLoading(true);
    _errorMessage = null;
    _successMessage = null;
    _fieldErrors = const {};
    _searchQuery = search ?? _searchQuery;
    _page = page;
    try {
      final result = await _repository.getEmployees(
        search: _searchQuery.isEmpty ? null : _searchQuery,
        page: _page,
      );
      _employees = result.data;
      _meta = result.meta;
      _links = result.links;
    } catch (error) {
      _errorMessage = 'Unable to load employees. Please try again.';
      _employees = const [];
      _meta = null;
      _links = null;
    } finally {
      _setLoading(false);
    }
  }

  Future<void> loadEmployee(String id) async {
    _setLoading(true);
    _errorMessage = null;
    _successMessage = null;
    try {
      _employee = await _repository.getEmployee(id);
    } catch (error) {
      _errorMessage = 'Unable to load employee details.';
      _employee = null;
    } finally {
      _setLoading(false);
    }
  }

  Future<bool> createEmployee(Employee employee) async {
    _setSubmitting(true);
    _errorMessage = null;
    _successMessage = null;
    _fieldErrors = const {};
    try {
      final created = await _repository.createEmployee(employee);
      _employee = created;
      _successMessage = 'Employee created successfully.';
      return true;
    } catch (error) {
      _handleError(error, fallbackMessage: 'Unable to create employee.');
      return false;
    } finally {
      _setSubmitting(false);
    }
  }

  Future<bool> updateEmployee(String id, Employee employee) async {
    _setSubmitting(true);
    _errorMessage = null;
    _successMessage = null;
    _fieldErrors = const {};
    try {
      final updated = await _repository.updateEmployee(id, employee);
      _employee = updated;
      _successMessage = 'Employee updated successfully.';
      return true;
    } catch (error) {
      _handleError(error, fallbackMessage: 'Unable to update employee.');
      return false;
    } finally {
      _setSubmitting(false);
    }
  }

  Future<bool> deleteEmployee(String id) async {
    _setSubmitting(true);
    _errorMessage = null;
    _successMessage = null;
    _fieldErrors = const {};
    try {
      await _repository.deleteEmployee(id);
      _successMessage = 'Employee deleted.';
      return true;
    } catch (error) {
      _handleError(error, fallbackMessage: 'Unable to delete employee.');
      return false;
    } finally {
      _setSubmitting(false);
    }
  }

  void clearMessages() {
    _errorMessage = null;
    _successMessage = null;
    _fieldErrors = const {};
    notifyListeners();
  }

  void _setLoading(bool value) {
    if (_loading == value) {
      return;
    }
    _loading = value;
    notifyListeners();
  }

  void _setSubmitting(bool value) {
    if (_submitting == value) {
      return;
    }
    _submitting = value;
    notifyListeners();
  }

  void _handleError(Object error, {required String fallbackMessage}) {
    if (error is DioException) {
      final message = _extractMessage(error) ?? fallbackMessage;
      _errorMessage = message;
      if (_isValidationError(error)) {
        _fieldErrors = _extractFieldErrors(error);
      }
      return;
    }

    if (error is ValidationException) {
      _errorMessage = error.message;
      return;
    }

    _errorMessage = fallbackMessage;
  }

  bool _isValidationError(DioException error) {
    return error.error is ValidationException || error.response?.statusCode == 422;
  }

  String? _extractMessage(DioException error) {
    final data = error.response?.data;
    if (data is Map<String, dynamic>) {
      final message = data['message'];
      if (message != null) {
        return message.toString();
      }
    }
    final apiError = error.error;
    if (apiError is ApiException) {
      return apiError.message;
    }
    return null;
  }

  Map<String, List<String>> _extractFieldErrors(DioException error) {
    final data = error.response?.data;
    if (data is Map<String, dynamic>) {
      final errors = data['errors'];
      if (errors is Map) {
        return errors.map((key, value) {
          final field = key.toString();
          if (value is List) {
            return MapEntry(
              field,
              value.map((item) => item.toString()).toList(),
            );
          }
          return MapEntry(field, [value.toString()]);
        });
      }
    }
    return const {};
  }
}
